	...
	public void init() {
		...
		try {
			...
			while(true) {
				...
				/*
				 클라이언트의 메세지를 모든 클라이언트에게 전달하기 위한 쓰레드생성 및 시작. 한명 당 하나씩의 쓰레드가 생성된다.
				 */
	...
	public void sendAllMsg(String name, String msg) {
		...
		while(it.hasNext()) {
			try {
				...
				/*
				 클라이언트에게 메세지를 전달한다.
				 매개변수로 name이 있는 경우와 없는 경우를 구분해서 메세지를 전달하게 된다.
				 */
				if(name.equals("")) {
					// 접속, 퇴장에서 사용되는 부분
					it_out.println(msg);
	...
	class MultiServerT extends Thread{
		...
		public void run() {
			...
			try {
				...
				out.println("현재 접속자 수는"+ClientMap.size()+"명 입니다.");
